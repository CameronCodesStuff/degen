...
### Fixed: intermittent 400 (Bad Request) errors on trade transactions
Root cause: the moon-boost math used everywhere pumps happen (`guaranteePumpToPositive100()`, `instantMoonBoost()`, `realityWarpBoost()`) solves `dUSD = √(targetPrice × k) − solReserve` directly. On a coin that's been pumped enough times in a row for its reserves to grow astronomically large, `targetPrice × k` can exceed float64's ~1.8×10³⁰⁸ ceiling and silently become `Infinity` — and the existing guard, `if(!(dUSD>0)) return;`, does **not** catch this, since `Infinity > 0` is `true` in JavaScript. That `Infinity` then flowed into `ammBuy()` and out into `newPrice`/`newSol`/`newTok`, and Firestore rejects non-finite numeric values outright with a 400 — which is exactly the error that was showing up, repeatedly, on `documents:commit`.

Fixed in three layers:
1. **`ammBuy()` itself** (the shared chokepoint for every buy in the app) now clamps `tokensOut` to 99% of the reserve regardless of what the raw math produces — mathematically `tokensOut` always stays below `tokenReserve` for any finite input, but at extreme ratios float64 precision can lose enough significant digits for the subtraction to round to zero or negative, which is what turns `newPrice` into `Infinity`/`NaN` in the first place.
2. **All three moon-boost functions** now check `isFinite(dUSD)` in addition to `dUSD>0`, and validate every downstream value (`newPrice`, `newSol`, `newTok`) is finite before it can reach a transaction write — aborting cleanly (silently skipping that boost) instead of writing corrupt data.
3. **Self-healing for anything already corrupted**: since `Infinity × anything` is still `Infinity`, a coin that got written with bad reserves *before* this fix existed would keep failing the same way forever on every future read. `botBuyOnCoin()`/`botSellOnCoin()` (the highest-frequency, most likely to touch a stuck coin soon) now check `isCoinHealthy()` first and repair the reserves back to a sane, unremarkable starting point if not, instead of attempting a doomed trade. Real user trades (`doBuy`/`doSell`) show a clear error ("price data looks corrupted, try again in a moment") instead of a confusing failure — the coin should self-heal automatically within the next bot tick or two, given how often the ambient loops touch things now.

**Follow-up round — the same gap existed in more places than the first pass caught.** The 400s kept happening after the fix above because it only covered the moon-boost functions and the bot trade paths, not `doBuy`/`doSell`'s own computation:
- **`doBuy`'s ownership-cap-clamped path** computes its own separate `finalUsd = (capTokens*coin.solReserve)/denom` formula, entirely independent of the moon-boost math — this can blow up the same way if `denom` ends up very small, and the result was never validated before reaching `ammBuy()`. Now checked for `isFinite`/positive before proceeding, and the final `tokensOut`/`newPrice`/`newSol`/`newTok` are all validated before the write, in both the normal and capped paths.
...
**Fixed: Hall of Legends showed nobody even for users who'd genuinely crossed $1Qi.** The achievement check only lived inside `refreshNetWorthSnapshot()`, which was only ever called right after a trade (or when visiting the Leaderboard). Someone who crossed the threshold *passively* — bank interest compounding, or coins they were already holding simply appreciating — without making a new trade since would never trigger it, so `legendAchievedAt` never got set despite their current net worth genuinely qualifying. Now also called once at sign-in, so every session re-evaluates regardless of how the wealth was gained.

**Abyss and the Singularity — walked back from sub-second ticks after they confirmed exactly the risk flagged when they were built.** They originally ran at 900ms (Abyss) and 450ms (Singularity's baseline), aiming for "at least once/twice a second." In practice this caused real, confirmed problems: console errors, 10+ second load times, and pages sometimes failing to load at all. The math behind why: every signed-in tab runs its own independent `setInterval` for both, so the load multiplies across every open tab, not just per-tab — a handful of people using the app at once meant a sustained double-digit number of transaction attempts *per second* against just two documents, indefinitely, for as long as anyone had a tab open. Now: **Abyss ticks every 3.5s**, **Singularity's baseline every 2s** — both still noticeably faster than every other coin in the app (Risky's 9s, the main Bot Market loop's ~22s), just no longer at a rate that compounds badly across multiple tabs. The "at least once/twice a second" framing from when these were built is no longer accurate and shouldn't be treated as a real guarantee.

**Fixed: the 10+ second load time itself.** Separately from the tick-rate issue above, `catchUpAllBotCoins()` — which runs on every single sign-in to fast-forward any bot coin that went stale while nobody had a tab open — was awaiting up to 15 coins **one at a time** in a sequential loop, each involving its own network round-trip. That's up to 15 sequential round-trips stacking up before the app felt usable, on every single page load. Changed to `Promise.all()` so all 15 run concurrently instead — same total work, but bounded by the slowest single one rather than the sum of all of them.

**The whole ambient bot economy now pauses when the tab isn't visible.** Every tick loop in the app — `botTick`, `botCoinTick`, `riskyCoinTick`, `mysteryCoinTick` (all via `scheduleNext()`'s guard) and the Abyss/Singularity `setInterval`s — already checked a `botRunning` flag before firing; that flag just never reacted to whether anyone was actually looking at the tab before now. A backgrounded or minimized tab was running every one of these at full speed for no visible benefit to anyone, which was real, wasted load contributing to the problems above. Wired up the Page Visibility API (`document.addEventListener('visibilitychange', ...)`): hide the tab, everything pauses completely (`stopBots()`); come back, it restarts (`startBots()`) and immediately re-runs the same catch-up sweep already used for "nobody had a tab open at all," so time spent hidden gets fast-forwarded the same way. Also handles a tab that starts already hidden (opened in the background) — it doesn't start the bot economy running until it's actually shown, since `visibilitychange` only fires on a transition, not the initial state.

**Immediate follow-up fix — this feature itself caused a fast burst of console errors on first deploy.** Some browsers and embedded/preview contexts don't fire `visibilitychange` as one clean transition — it can fire rapidly or spuriously. Without protection, that meant repeated `stopBots()`/`startBots()` cycles in quick succession, each one kicking off a fresh `catchUpAllBotCoins()` that could overlap with a still-in-flight previous call — many concurrent transactions landing on the same handful of coins at once, which produces exactly a fast burst of errors. Two fixes, defense in depth: the visibility handler is now debounced (800ms — only reacts once visibility has actually been stable for a moment, not on every intermediate flicker), and `catchUpAllBotCoins()` now guards against running twice concurrently regardless of what triggers it.
