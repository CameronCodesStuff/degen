...
**Stats menu**: a "📊 View Stats" button on the profile panel opens a modal showing total spent, count of coins sniped, a live current-value tally (only for whatever you still hold from the sniped list — computed with a handful of reads after the modal opens, not blocking it), and the last 30 coins sniped into with amount and time. Since a sniped coin's holding can also get topped up or partly sold by hand afterward, "current value" isn't a perfectly isolated snipe-only P&L — it's described that way in the modal itself rather than pretending otherwise.
