...
**Abyss and the Singularity, much faster now.** The Abyss ticks every 900ms at a 98% trade chance — reliably at least once a second, not just averaging close to it. The Singularity's baseline tick runs every 450ms (aiming for at least twice a second), layered under the real-activity mirror mechanism described above — real mirrored trades still drive the bulk of its behavior whenever real trading is actually happening (the baseline stays silent for 400ms after any real mirror fires), and only fills in with small, direction-neutral filler trades when real activity goes quiet for a moment. Both bypass the shared jittered scheduler entirely (`scheduleNext`'s ±3000ms randomization is fine at tick rates measured in seconds-to-minutes, but would completely break a per-second guarantee at this timescale — could even land negative) in favor of plain, precise `setInterval` loops.

**Worth knowing, honestly**: sustained writes at roughly 1-3 times a second to a *single* Firestore document (which is what both of these now are) sits right at Firestore's commonly-cited soft guidance of about one write per second per document before contention risk goes up — concurrent transactions competing for the same doc more often, and slightly higher latency on each one. This is a different kind of risk than the earlier rate-limit issue (that was too many separate writes spread across the whole app; this is concentrated on two specific documents), and `runTransaction` retries automatically on contention rather than failing outright, but it's worth keeping an eye on if these two coins specifically start feeling laggy or erroring under real use. Dialing the tick rate back on either one is the fix if that happens.
