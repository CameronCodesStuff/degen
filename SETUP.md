...
- **Guaranteed thousands of trades from the moment it appears** — always uses the "established" backstory path (never the ~30%-chance brand-new-with-zero-trades version other bot coins can get), seeded with 3,000–9,000 trades instead of the normal 900–6,100 range.
...
- **Only goes up, with the occasional small drop** — instead of the normal trend-bias logic (which can genuinely swing bearish), these coins trade on a fixed heavily-bullish split: ~93% buy chance normally, dropping to ~30% for a shallow dip roughly 12% of the time (and even then, the sell size is capped smaller than a normal dump) — never a real reversal, just enough wobble to not look robotic. The fabricated backstory chart is generated the same way, mostly-upward drift with rare small dips, so it never has a "this already crashed once" look even in its fake past.
